...
export function BoardDetailsHeader({ board, filterBy, viewType, setViewType, onToggleBoardMenu }) {
    ...
    <section className="board-details-header">
      <div className="board-topbar-start">
        <div className="board-title-container">
          <h1 className="board-details-title" onClick={handleTitleClick} ref={boardTitleH1}>
            {board.title}
          </h1>
          <input
        ...
        <Button
          className={`board-details-header-button dynamic-button${viewType === "board" ? " active" : ""}`}
        ...
        <Button
          className={`board-details-header-button dynamic-button${viewType === "dashboard" ? " active" : ""}`}
      ...
      <div className="board-topbar-end">
        <span className={`filter-btns-container flex ${!isFilterEmpty || isPopoverOpen ? "active" : ""}`}>
          <button className="filter-btn button dynamic-button" onClick={handleClick}>
        ...
        <ul className="member-list">
          {board.members.map((member) => {
            return (
              <li key={member.id} className="member-item" draggable="true" onDragStart={(e) => handleMemberDragStart(e, member)}>
