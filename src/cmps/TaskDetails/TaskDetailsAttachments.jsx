...
export function TaskDetailsAttachments({ task, onUpdateTask }) {
  ...
  async function onUpdateTaskCover(attachmentImgUrl) {
    const taskStyleToUpdate = {}

    if (attachmentImgUrl !== task?.style?.bgImg) {
      const dominantColor = await uiService.getDominantColor(attachmentImgUrl)
      ...
      <ul className="attachment-list clean-list">
        {task.attachments.map((attachment) => (
          <li className="attachment-item" key={attachment.id}>
            <div className="thumbnail" style={{ backgroundImage: `url(${attachment.imgUrl})` }}></div>
            <div className="attachment-details">
              <Link to={attachment.imgUrl} target="_blank">
                {minimizeImgUrl(attachment.imgUrl)}
              </Link>
              <article className="actions">
                <span>Added {dayjs(attachment.createdAt).format("MMM D YYYY [at] h:mm A")}</span>
                <Button onClick={() => onRemoveAttachment(attachment.id)}>
                  <span>Delete</span>
                </Button>
                <Button variant="link" onClick={() => onUpdateTaskCover(attachment.imgUrl)}>
                  <SvgIcon iconName={"cover"} />
                  <span>{attachment.imgUrl !== task?.style?.bgImg ? "Make cover" : "Remove cover"} </span>
                </Button>
              </article>
            </div>
          </li>
